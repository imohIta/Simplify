...
$registry->get('authenticator')->checkPrivilege($thisUser->get('activeAcct'), array(1,8,9,11), true);
...
/*if(!$registry->get('session')->read('itemTypes')){
    $registry->get('session')->write('itemTypes', Item::fetchTypes());
}*/


# get all items from this user's Stock execept 
# pass true as argument if stock items that are finished are not to be fetched
//if(!$registry->get('session')->read('items')){ 
//    $registry->get('session')->write('items', Item::fetchAll('',true));
//}

$items = Item::fetchAll('',true);
                        	...
                        	<table style="width:100%">
                        		<tr>
                        		 ...
                        		 <td style="width:35%" align="center">
                        		 	<select class="form-control chosen-select" id="item" data-placeholder="Select Item" style="width:220px" onChange="setItemPrice(this.value);">
                                      	  <option></option>
                                      	  <?php foreach ($items as $row) {
                                      	    $posItem = new PosItem(new Item($row->itemId));
                                      	  ?>
		      ...
		      <div class="modal-dialog">
		        <div class="modal-content">
		          ...
		          <div class="modal-body">
		            <p>Are you sure yow want to delete this item from the temporary sales list? .</p>
