...
class BaseView extends FuniObject implements SplObserver{
	...
	public final function render($tmpl = "dashboard"){
		...
		if(!file_exists($path)){
			Error::throwException($tmpl . ' Template Not Found');
		}
	...
	*/
	public final function display($widget, $msg, $tmpl){
		...
		if(!file_exists($path)){
			Error::throwException($widget .' Widget Not Found');
		}
		ob_start();
		include $path;
		$r = ob_get_clean();
		...
		if(!$tmpl){
			...
			#set the minifier templete content to a session and echo it in the caling page
